...
- After converting a prototype to EDS (the `stardust-to-snowflake` skill's Step 10) — use `--profile eds`.
...
|---|---|---|---|
| **Pixel / layout** | `tools/da/visual-diff.mjs` | stretched images, dropped max-width wraps, blank renders, surface/ground colour flips, image-count gaps | "right text, wrong slot"; a dropped CTA (full pixels, plausible colours → no flag) |
| **Structural content + type** | `tools/da/content-diff.mjs` | MISSING / ROLE-SWAPPED headings·eyebrows·CTAs, invented/dropped body copy, rendered-FACE font forks (width probe) | geometry / layout regressions |
...
# 1. PIXEL/layout
node tools/da/visual-diff.mjs   "$PROTO" "$BUILD" --profile eds --sections ".hero"

# 2. STRUCTURAL content + type
node tools/da/content-diff.mjs  "$PROTO" "$BUILD" --profile eds   # --json dumps both inventories
```
...
`tools/da/diff-profiles.mjs` holds them. A profile supplies the source/target **labels**,
...
- **`eds`** (default) — Edge Delivery / DA remediation language + `stardust-to-snowflake` finding numbers.
...
`stardust-to-snowflake` conversion workflow's Validate phase runs both after building
...
> Naming note: this is a project skill, invoked as `/stardust-diff`. The `stardust:diff`
> colon form is the plugin-namespaced equivalent — only available if these are packaged
> as a `stardust` plugin. The behaviour is identical either way.
